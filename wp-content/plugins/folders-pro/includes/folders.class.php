...
//        }
    ...
    public function wcp_folders_by_order(){
        ...
        if ($errorCounter == 0) {
            ...
            $tree_data = WCP_Pro_Tree::get_full_tree_data($folder_type, $order_by, $order);
    ...
    public function show_dropdown_on_media_screen() {
        if(self::is_for_this_post_type('attachment')) {
            ...
            /* Free/Pro Class name change */
            $options = WCP_Pro_Tree::get_folder_option_data($post_type);?>
    ...
    public function output_backbone_view_filters() {
        ...
        if ($typenow == "attachment" || !is_admin()) {
            ...
            /* Free/Pro URL Change */
            wp_enqueue_script( 'folders-media', WCP_PRO_FOLDER_URL.'assets/js/media.js', array( 'media-editor', 'media-views' ), false, true );
...
//        }
    ...
    public function admin_footer()
    {
        if (self::is_active_for_screen()) {
            ...
            /* Do not change: Free/Pro Class name change */
            $tree_data = WCP_Pro_Tree::get_full_tree_data($folder_type);
            ...
            $form_html = WCP_Pro_Forms::get_form_html($terms_html, $hasValidKey, $activateURL);
...
//        }
    ...
    function folders_admin_styles()
    {
        if (self::is_active_for_screen()) {
            ...
            wp_enqueue_style('wcp-folders-admin', plugin_dir_url(dirname(__FILE__)) . 'assets/css/design.min.css', array(), WCP_PRO_FOLDER_VERSION);
    ...
    function folders_admin_scripts($hook)
    {
        if (self::is_active_for_screen()) {
            ...
            /* Free/Pro Version change */
            wp_enqueue_script('wcp-folders-mcustomscrollbar', plugin_dir_url(dirname(__FILE__)) . 'assets/js/jquery.mcustomscrollbar.min.js', array(), WCP_PRO_FOLDER_VERSION);
            wp_enqueue_script('wcp-folders-jstree', plugin_dir_url(dirname(__FILE__)) . 'assets/js/jstree.min.js', array('jquery'), WCP_PRO_FOLDER_VERSION);
            wp_enqueue_script('wcp-jquery-touch', plugin_dir_url(dirname(__FILE__)) . 'assets/js/jquery.ui.touch-punch.min.js', array('jquery'), WCP_PRO_FOLDER_VERSION);
            wp_enqueue_script('wcp-folders-custom', plugin_dir_url(dirname(__FILE__)) . 'assets/js/folders.min.js', array('jquery', 'jquery-ui-resizable', 'jquery-ui-draggable', 'jquery-ui-droppable', 'jquery-ui-sortable', 'backbone'), WCP_PRO_FOLDER_VERSION);
//            wp_enqueue_script('wcp-folders-custom', plugin_dir_url(dirname(__FILE__)) . 'assets/js/custom.js', array('jquery', 'jquery-ui-resizable', 'jquery-ui-draggable', 'jquery-ui-droppable', 'jquery-ui-sortable', 'backbone'), WCP_PRO_FOLDER_VERSION);
        ...
        if($hook == "media-new.php") {
            if(self::is_for_this_post_type('attachment') || self::is_for_this_post_type('media')) {
                ...
                wp_enqueue_script('wcp-folders-add-new-media', plugin_dir_url(dirname(__FILE__)) . 'assets/js/new-media.js', array('jquery'), WCP_PRO_FOLDER_VERSION);
    ...
    public function admin_menu() {
        ...
        if (isset($customize_folders['show_folder_in_settings']) && $customize_folders['show_folder_in_settings'] == "yes") {
            ...
            add_submenu_page(
                $menu_slug,
                esc_attr__('License Key', WCP_FOLDER),
                esc_attr__('License Key', WCP_FOLDER),
                'manage_options',
    ...
    public function wcp_folders_settings($page) {
        ...
        $setting_page = in_array($setting_page, array("folder-settings", "customize-folders", "folders-import", "license-key"))?$setting_page:"folder-settings";
        ...
        if (self::is_active_for_screen()) {
            ...
            if (empty($license_data)) {
                ...
                <div class="notice notice-warning is-dismissible">
                    <p><?php esc_attr_e("To receive updates, Please provide your valid", WCP_FOLDER)?> <a href='<?php echo esc_url($url) ?>'><?php esc_attr_e("license key", WCP_FOLDER) ?></a></p>
                </div>
                <?php
            } else {
                if ($license_data['license'] == "expired") {
                    ...
                    <div class="notice notice-error is-dismissible">
                        <p><?php esc_attr_e("Your license key has been expired, Please renew your ", WCP_FOLDER) ?><a href='<?php echo esc_url($url) ?>'>license key</a><?php esc_attr_e(" to receive updates.", WCP_FOLDER) ?></p>
                    </div>
                    <?php
                }
            }
    ...
    public function download_folder() {
        if (current_user_can('manage_options') || current_user_can("edit_pages") || current_user_can("edit_posts")) {
            $folder_id = filter_input(INPUT_GET, 'folder_id', FILTER_SANITIZE_STRING);
